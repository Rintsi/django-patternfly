...
class FieldRenderer(BaseRenderer):
    ...
    def __init__(self, field, *args, **kwargs):
        ...
        # These are set in Django or in the global PATTERNFLY settings, and
    ...
    def add_class_attrs(self, widget=None):
        ...
        if ReadOnlyPasswordHashWidget is not None and isinstance(widget, ReadOnlyPasswordHashWidget):
            ...
            classes = add_css_class(classes, "pf-c-form-control", prepend=True)
    ...
    def list_to_class(self, html, klass):
        ...
        # Apply patternfly classes to labels and inputs.
        # A simple 'replace' isn't enough as we don't want to have several
        # 'class' attr definition, which would happen if we tried to
        # 'html.replace("input", "input class=...")'
    ...
    def fix_date_select_input(self, html):
        ...
        return '<div class="row patternfly-multi-input">{html}</div>'.format(html=html)
    ...
    def make_input_group(self, html):
        if self.is_input_group:
            ...
            html = '<div class="pf-c-form__group">{html}</div>'.format(html=html)
        return html
    ...
    def append_help(self, html):
        ...
        if field_help:
            help_html = render_template_file(
                "patternfly/field_help_text.html",
                context={
                    "field": self.field,
    ...
    def add_label(self, html):
        ...
        if label:
            html = render_label(label, required, label_for=self.field.id_for_label, label_class=self.get_label_class()) + html
        return html
