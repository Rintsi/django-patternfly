...
@register.simple_tag
def patternfly_css_additions_url():
    """
    Return the full url to the Bootstrap CSS library.
    ...
    **Example**::
        ...
        {% bootstrap_css_url %}
    """
    return css_additions_url()

@register.simple_tag
def patternfly_theme_url():
    """
...
def patternfly_css():
    ...
    **Example**::
    ...
    """
    rendered_urls = []
    if patternfly_css_url():
        rendered_urls.append(render_link_tag(patternfly_css_url()))
    if patternfly_css_additions_url():
        rendered_urls.append(render_link_tag(patternfly_css_additions_url()))
    if patternfly_theme_url():
        rendered_urls.append(render_link_tag(patternfly_theme_url()))
...
def bootstrap_formset_errors(*args, **kwargs):
    ...
    **Parameters**::
        ...
        layout
            Context value that is available in the template ``patterfly/form_errors.html`` as ``layout``.
...
@register.simple_tag
def patternfly_form(*args, **kwargs):
    """
    Render a form.
...
def bootstrap_form_errors(*args, **kwargs):
    ...
    **Parameters**::
        ...
        layout
            Context value that is available in the template ``patternfly/form_errors.html`` as ``layout``.
...
@register.simple_tag
def patternfly_field(*args, **kwargs):
    """
    Render a field.
...
@register.simple_tag
def patternfly_button(*args, **kwargs):
    """
    Render a button.
    ...
    **Usage**::
        ...
        {% patternfly_button content %}

    **Example**::

        {% patternfly_button "Save" button_type="submit" button_class="btn-primary" %}
    """
    return render_button(*args, **kwargs)


@register.simple_tag
def patternfly_alert(content, alert_type="info", dismissible=True):
    """
    Render an alert.
...
def patternfly_buttons(parser, token):
    """
...
class ButtonsNode(template.Node):
    ...
    def render(self, context):
        ...
        if submit:
            buttons.append(patternfly_button(submit, "submit"))
        if reset:
            buttons.append(patternfly_button(reset, "reset"))
        buttons = " ".join(buttons) + self.nodelist.render(context)
        output_kwargs.update({"label": None, "field": buttons})
        css_class = output_kwargs.pop("form_group_class", "pf-c-form__group pf-m-action")
...
def bootstrap_messages(context, *args, **kwargs):
    ...
    Uses the template ``patternfly/messages.html``.
    ...
    **Example**::
    ...
    return render_template_file("patternfly/messages.html", context=context)


@register.inclusion_tag("patternfly/pagination.html")
